...
print("Proportion of Key Eavesdropped: ",  0.344)
prob = 0
for i in range(7):
    for j in range(7):
        if i == j:
        ...
        if i in triangle:
            ...
            if j in triangle:
                ...
                factortwo = 1/7
        prob += factorone * factortwo * (pow(math.cos(compare[i][j]/2 * math.pi/180), 2))
    ...
    for j in range(7):
        for k in range(7):
            ...
            if k in triangle:
                ...
                factorthree = 1/8
            prob += factorone * factortwo * factorthree * (pow(math.cos(compare[i][k]/2 * math.pi/180), 2) * pow(math.cos(compare[k][j]/2 * math.pi/180), 2) + (1 - pow(math.cos(compare[i][k]/2 * math.pi/180), 2)) * (1 - pow(math.cos(compare[k][j]/2 * math.pi/180), 2)))
print("Proportion of Checked Bits Match (Interference + Known Different Basis Schemes): ", prob)